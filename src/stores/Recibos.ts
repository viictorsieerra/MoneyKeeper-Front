...
export const useReciboStore = defineStore('recibo', () => {
  ...
  function findByUser() {
    ...
    fetch("https://moneykeeper-api.retocsv.es/Recibo/recibos", {
  ...
  const createRecibo = async (nuevoRecibo: { _idCuenta: number, _nombreRecibo: string, _dineroRecibo: number, _activa: boolean, _fecRecibo: string }) => {
    ...
    try {
      ...
      const response = await fetch('https://moneykeeper-api.retocsv.es/Recibo', {
        method: 'POST',
        headers: {
  ...
  const deleteRecibo = async (idRecibo: number) => {
    ...
    try {
      ...
      const response = await fetch(`https://moneykeeper-api.retocsv.es/Recibo/${idRecibo}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${strToken}`,
        }
